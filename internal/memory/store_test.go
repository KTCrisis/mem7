...
func TestMultiplicativeScoring(t *testing.T) {
	s := newStore(t)
	s.SetScoring("multiplicative")
	...
	if idxRecent > idxOld {
		t.Fatalf("multiplicative scoring: recent should rank above old\n%s", text)
	}
}

func TestAccessTrackingBoost(t *testing.T) {
	s := newStore(t)
	s.SetScoring("multiplicative")

	now := time.Now().UTC()
	var popularID int64
	for _, key := range []string{"popular", "obscure"} {
		f := fact{
			Entity:    key,
			Predicate: "note",
			Object:    "kubernetes cluster configuration details",
	...
	for i := 0; i < 10; i++ {
		_ = s.index.TouchAccessed([]int64{popularID})
	}

	res := s.ToolSearch(map[string]any{"query": "kubernetes cluster"})
	text := getText(t, res)

	idxPop := strings.Index(text, "popular")
	idxObs := strings.Index(text, "obscure")
	if idxPop < 0 || idxObs < 0 {
		t.Fatalf("expected both, got: %s", text)
	}
	if idxPop > idxObs {
		t.Fatalf("access boost failed: popular should rank above obscure\n%s", text)
	}
}
...
func TestAccessTrackingIncrementsOnRecall(t *testing.T) {
	...
	call(t, s, "memory_recall", map[string]any{"key": "tracked"})
	call(t, s, "memory_recall", map[string]any{"key": "tracked"})
	call(t, s, "memory_recall", map[string]any{"key": "tracked"})

	results, err := s.index.Query(filter{Entity: "tracked", Limit: 1})
	if err != nil || len(results) == 0 {
		t.Fatal("expected tracked entry")
	}

	var count int
	row := s.index.(*sqliteStore).db.QueryRow("SELECT access_count FROM facts WHERE id = ?", results[0].ID)
	if err := row.Scan(&count); err != nil {
		t.Fatal(err)
	}
	if count != 3 {
		t.Fatalf("expected access_count=3, got %d", count)
	}
}
